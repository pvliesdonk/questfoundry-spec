...
- Ensure `04-protocol/INTENTS.md` formally references `envelope.schema.json`, defines required
  envelope fields per intent, and standardizes error responses.

Tasks

- In `04-protocol/INTENTS.md`:
  - For each intent, add an “Envelope requirements” block: mandatory context fields, safety
    expectations, sender/receiver roles, and payload.type mapping.
  - Define `ack` and `error` envelope shapes by reference to envelope.schema.json.
  - Ensure error taxonomy matches envelope schema (`validation_error`, `business_rule_violation`,
