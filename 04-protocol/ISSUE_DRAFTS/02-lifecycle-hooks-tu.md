...
- Codify allowed state transitions for hooks and TUs, who triggers them, and required
  intents/payloads.

Tasks

- Add `04-protocol/LIFECYCLES/hooks.md`:
  - Lifecycle: `proposed → accepted → in-progress → resolved → canonized`; branches `deferred`,
    `rejected`.
  - For each transition: allowed senders (SR/owner), required intents, payload schemas, errors on
    invalid transitions.
...
- Each lifecycle lists transitions with: preconditions, sender role, intent name(s), required
  payload `$schema`, and error cases.
