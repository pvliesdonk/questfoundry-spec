...
**Purpose:** Define the 7 mandatory checks that all Cold merges and exports must pass.
...
- Logs never appear on player surfaces (Presentation bar)
...
**Note:** This bar is conditional - only applies when determinism is explicitly promised

### 7. Presentation Safety

**What it checks:** No spoilers/meta/technique on surfaces, accessibility baseline met

**Specific validations:**
- No codewords, internal IDs, gate logic on player surfaces
- No technique talk (seeds, models, DAW settings, schema names) on surfaces
- Alt text present for images
- Captions/text equivalents for audio
- High-contrast, readable text
- Diegetic refusals (gates enforced in-world)

**Who enforces:** Gatekeeper (primary), all roles (player-safe surfaces)

**Fails look like:** Codeword on surface, "Option locked", missing alt text, technique leak
...
**Decision Criteria:**
- All 7 bars must be green (or explicitly excepted) before Cold merge
