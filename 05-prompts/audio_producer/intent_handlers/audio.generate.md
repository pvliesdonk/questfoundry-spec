...
STATUS: SCAFFOLD
TODO: Define render settings, batch handling, and review checklist. Note: Internal procedure (no new protocol intent).
...
Process
1) For each cue: select provider(s) and settings (model/voice/version, tempo/key, FX chain; seed if deterministic).
